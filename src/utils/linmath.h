...
static inline float* vec3_scale_r(vec3 v, float s)
{
	int i;
	static float r[3];
	for(i=0; i<3; ++i)
...
static inline float* vec3_norm_r(vec3 v)
{
	float k = 1.f / vec3_len(v);
	return vec3_scale_r(v, k);
}
...
/*static inline void mat4x4_scale(mat4x4 M, mat4x4 a, float s
{
	int i;
	for(i=0; i<4; ++i)
...
static inline void mat4x4_scale(mat4x4 M, mat4x4 a, float s)
{
	vec4_scale(M[0], a[0], s);
	vec4_scale(M[1], a[1], s);
	vec4_scale(M[2], a[2], s);
}
